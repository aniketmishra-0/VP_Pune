...
export default function TimetableViewer({ adminHeaders }: TimetableViewerProps) {
  ...
  const getName = (code: string) => teacherNames[code] || "";
  ...
  const fetchTeacherNames = useCallback(async () => {
    try {
      ...
      if (r.ok) { const d = await r.json(); if (d.teachers) setTeacherNames(d.teachers); }
    } catch {}
  }, [adminHeaders]);
  ...
  const handleSearchInput = (val: string) => {
    ...
    // Don't auto-select — let them pick from suggestions
  };
  ...
  // Search suggestions — match by code or name
  ...
  }, [allCodes, searchQuery, teacherNames]);

  // Chips (only when no selection)
  const MAX = 14;
  ...
  return (
    <div className="pb-28">
      ...
      <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200/50 dark:border-gray-800/40 shadow-sm overflow-hidden">
          ...
          <AnimatePresence>
            {showSuggestions && suggestions.length > 0 && !selectedCode && (
              <motion.div
                ...
                className="absolute left-3 right-3 md:left-4 md:right-4 top-full mt-1 bg-white dark:bg-[#1a2332] rounded-xl border border-slate-200 dark:border-gray-700 shadow-xl z-50 overflow-hidden max-h-[280px] overflow-y-auto"
              >
                {suggestions.map((c, i) => {
                  ...
                  return (
                    <button
                      key={c.code}
                      onClick={() => selectTeacher(c.code)}
                      className={`w-full flex items-center justify-between px-3 py-2.5 text-left cursor-pointer hover:bg-slate-50 dark:hover:bg-gray-800/60 transition-colors ${
                        i !== suggestions.length - 1 ? "border-b border-slate-100 dark:border-gray-800/40" : ""
                      }`}
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="w-8 text-center text-[11px] font-black text-[#5277f7] font-mono bg-[#5277f7]/10 rounded px-1 py-0.5 shrink-0">{c.code}</span>
                        {name && <span className="text-[11px] text-slate-600 dark:text-slate-300 truncate">{name}</span>}
                        {!name && <span className="text-[11px] text-slate-400 italic">Unknown</span>}
                      </div>
                      <span className="text-[9px] text-slate-400 shrink-0 ml-2">{c.count} lec</span>
                    </button>
                  );
                })}
                {allCodes.filter(c => {
                  const q = searchQuery.toLowerCase();
                  return c.code.toLowerCase().includes(q) || getName(c.code).toLowerCase().includes(q);
                }).length > 8 && (
                  <div className="px-3 py-1.5 text-center text-[9px] text-slate-400 bg-slate-50 dark:bg-gray-900/30">
                    + more results...
                  </div>
                )}
              </motion.div>
        ...
        {/* Chips — only when NO code selected */}
        {!selectedCode && (
            ...
            <div className="flex flex-wrap gap-1">
              {chipList.map(c => (
                <button key={c.code} onClick={() => selectTeacher(c.code)}
                  title={getName(c.code) ? `${c.code} — ${getName(c.code)}` : c.code}
          ...
          <div className="border-t border-slate-100 dark:border-gray-800/40 px-3 py-2 md:px-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {getName(selectedCode) && <span className="text-[10px] text-slate-400 truncate hidden sm:inline">· {getName(selectedCode)}</span>}
                  </div>
                  {getName(selectedCode) && <p className="text-[9px] text-slate-400 truncate sm:hidden">{getName(selectedCode)}</p>}
                </div>
              </div>
              <div className="flex items-center gap-2.5 text-[10px] text-slate-400 shrink-0">
                ...
                <span className="hidden sm:flex items-center gap-0.5"><Layers className="w-3 h-3 text-amber-500" />{stats.batches}</span>
      ...
      {!selectedCode && !isDemo && allCodes.length > 0 && (
        <div className="mt-4 text-center py-6">
          ...
          <p className="text-xs text-slate-400">Search or select a teacher code</p>
        </div>
      )}
