              ...
              {(() => {
                ...
                return (
                  <div className="space-y-6">
                    ...
                    {adminTab === "timetable" && (() => {
                      const [ttUrlSaved, setTtUrlSaved] = React.useState<null | "saving" | "saved" | "error">(null);
                      const [ttUrlMsg, setTtUrlMsg] = React.useState("");
                      const [teacherList, setTeacherList] = React.useState<Array<{code: string; name: string}>>([]);
                      const [newCode, setNewCode] = React.useState("");
                      const [newName, setNewName] = React.useState("");
                      const [teacherSaved, setTeacherSaved] = React.useState<null | "saving" | "saved" | "error">(null);
                      const [teacherMsg, setTeacherMsg] = React.useState("");
                      ...
                      React.useEffect(() => {
                        (async () => {
                          try {
                            ...
                            if (r.ok) {
                              const d = await r.json();
                              const el = document.getElementById("timetable-url-input") as HTMLInputElement;
                              if (el && d.url) el.value = d.url;
                            }
                          } catch {}
                        })();
                      }, []);
                      ...
                      return (
                        <div className="space-y-4 max-w-2xl">
                          ...
                          <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200/50 dark:border-gray-800/40 p-4 md:p-5 shadow-sm space-y-3">
                            ...
                            <div className="flex gap-2">
                              <input type="text" id="timetable-url-input"
                            ...
                            <button
                              onClick={async () => {
                                try {
                                  ...
                                  setTtUrlSaved("saved"); setTtUrlMsg("Refreshing data in background...");
                                  setTimeout(() => setTtUrlSaved(null), 3000);
                                } catch {}
                              }}
                              ...
                              <div className="max-h-[300px] overflow-y-auto space-y-1 pr-1">
                                {teacherList.map(t => (
                                  <div key={t.code} className="flex items-center justify-between bg-slate-50 dark:bg-gray-900/30 rounded-lg px-3 py-2 group">
                                    ...
                                    <button onClick={() => removeTeacher(t.code)}
